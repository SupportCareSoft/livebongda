...
export const overlayUrl = new URL(`overlay.html?session=${session}`, location.href).href;
export const controllerUrl = new URL(`controller.html?session=${session}`, location.href).href;
export const getElapsed = state => state.running ? Math.max(0, state.elapsed + Math.floor((Date.now() - state.startedAt) / 1000)) : state.elapsed;
export const clockText = seconds => `${String(Math.floor(seconds / 60)).padStart(2, '0')}:${String(seconds % 60).padStart(2, '0')}`;
...
export function watchState(callback, onError) {
  if (!configured) { onError?.('Chưa có cấu hình Firebase.'); return () => {}; }
  return onValue(gameRef, snapshot => callback({ ...defaults, ...(snapshot.val() || {}) }), onError);
}

export async function saveState(next) {
  if (!configured) throw new Error('Chưa có cấu hình Firebase.');
  await set(gameRef, { ...defaults, ...next, updatedAt: Date.now() });
}
