...
function render() {
 ...
 $('startPause').textContent = state.running ? 'TẠM DỪNG' : 'BẮT ĐẦU';
  if (document.activeElement !== $('matchTitle')) $('matchTitle').value = state.title || defaults.title;
...
function setup() {
  ...
  for (const [id, key, fallback] of textFields) $(id).addEventListener('input', () => { clearTimeout(nameTimer); nameTimer = setTimeout(() => update({ [key]: $(id).value.trim() || fallback }), 350); });
